import { getLanguageMetadata, getLevelMetadata, getSectionMetadata } from "@/lib/content";
...
export default async function GermanHome() {
  ...
  // Get first lesson info for each enabled level (for fallback navigation)
  const levelFirstLessons: Record<string, { levelId: string; sectionId: string; lessonId: string }> = {};

  for (const level of languageMeta.availableLevels.filter((l) => l.enabled)) {
    try {
      const levelMeta = await getLevelMetadata("german", level.id);
      const firstSection = levelMeta.sections.find((s) => s.enabled);
      if (firstSection) {
        const sectionMeta = await getSectionMetadata("german", level.id, firstSection.id);
        const firstLesson = sectionMeta.lessons[0];
        if (firstLesson) {
          levelFirstLessons[level.id] = {
            levelId: level.id,
      ...
      console.error(`Error fetching first lesson for level ${level.id}:`, error);
    }
  }
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {languageMeta.availableLevels.map((level) =>
                  level.enabled && levelFirstLessons[level.id] ? (
                    <LevelLink
                      key={level.id}
                      levelId={level.id}
                      levelName={level.name}
                      languageCode="german"
                      firstLesson={levelFirstLessons[level.id]}
                    />
                  ) : (
                    <div key={level.id} className="relative bg-gradient-to-br from-gray-100 to-gray-50 dark:from-gray-800/50 dark:to-gray-900/50 rounded-2xl p-8 shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden h-full">
                      {/* Top accent line */}
                      ...
                      <div className="relative opacity-50">
                        {/* Header */}
                        <div className="mb-6">
                          <h3 className="text-3xl font-bold text-gray-500 dark:text-gray-400 mb-1 text-left">{level.name}</h3>
